...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "API Route: Slack webhook URL:",
      SLACK_WEBHOOK_URL ? "Configured" : "NOT CONFIGURED"
    );
    console.log(
      "API Route: Slack message to send:",
    ...
    if (!SLACK_WEBHOOK_URL) {
      ...
      return NextResponse.json(
        { error: "Slack webhook not configured on server" },
        { status: 500 }
      );
    }

    console.log("API Route: Sending to Slack webhook...");
    ...
    console.log("API Route: Slack response status:", slackResponse.status);
    console.log("API Route: Slack response text:", await slackResponse.text());
